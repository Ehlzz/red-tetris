...
function checkLines(player, socket) {
    ...
    const newGrid = oldGrid.filter(
        row => !row.every(cell => cell !== null && cell !== 'indestructible')
    );

    const linesCleared = oldGrid.length - newGrid.length;
...
function setHoverBlock(player) {
    ...
    player.grid = player.grid.map(row =>
        row.map(cell => (cell === 'hover' ? null : cell))
    );
    ...
    for (let y = 0; y < player.currentBlock.shape.length; y++) {
        for (let x = 0; x < player.currentBlock.shape[y].length; x++) {
            ...
            if (gridY < 0 || gridY >= 22 || gridX < 0 || gridX >= 10) continue;
            if (player.grid[gridY][gridX] !== null) continue;
