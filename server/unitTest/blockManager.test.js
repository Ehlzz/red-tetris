...
import { fixBlock, checkLines, setHoverBlock } from '../game/blockManager.js';
  ...
  describe('setHoverBlock', () => {
    beforeEach(() => {
      isCollision.mockImplementation((player, direction) => {
        return player.position.y + direction.y >= 21;
      });
    });
    ...
    it('should set hover block at bottom position', () => {
      ...
      const hoverCells = mockPlayer.grid.flat().filter(c => c === 'hover');
    ...
    it('should handle block at grid edge', () => {
      ...
      const hoverCount = mockPlayer.grid.flat().filter(c => c === 'hover').length;
